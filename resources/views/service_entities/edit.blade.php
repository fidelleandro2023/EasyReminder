<x-app-layout>
    ...
    <div class="container mx-auto py-8">
        <h1 class="text-2xl font-bold mb-6">Editar Entidad de Servicio</h1> 
        <form action="{{ route('service_entities.update', $serviceEntity->id) }}" method="POST" class="bg-white p-6 shadow rounded-lg space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="name" id="name" 
                    ...
                    value="{{ old('name', $serviceEntity->name) }}" placeholder="Ejemplo: Agua, Luz, Teléfono">
                @error('name')
            ...
            <div>
                ...
                <textarea name="description" id="description" 
                    class="w-full border-gray-300 rounded-lg focus:ring focus:ring-blue-200" 
                    placeholder="Breve descripción del servicio">{{ old('description', $serviceEntity->description) }}</textarea>
                @error('description')
                <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
                ...
                <select name="parent_id" id="parent_id" class="w-full border-gray-300 rounded-lg focus:ring focus:ring-blue-200">
                    ...
                    @foreach ($categories as $category)
                        <option value="{{ $category->id }}" class="font-bold" data-category="{{ $category->name }}"
                            {{ $serviceEntity->parent_id == $category->id ? 'selected' : '' }}>
                            {{ $category->name }}
                        </option>
                        @foreach ($category->services as $service)
                            <option value="{{ $service->id }}" data-category="{{ $category->name }}"
                                {{ $serviceEntity->parent_id == $service->id ? 'selected' : '' }}>
                                &nbsp;&nbsp;&nbsp;{{ $service->name }}
                            </option>
                        @endforeach
                    @endforeach
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#parent_id').select2({
                placeholder: 'Seleccione un servicio padre',
                allowClear: true,
                width: '100%',
